...
public class AuthApiController implements AuthApi {
    ...
    public ResponseEntity<LoginResponse> login(@Valid LoginRequest loginRequest) {
        ...
        try {
            ...
            log.debug("login failed username={}, reason={}", loginRequest.getUsername(), exception.getMessage());
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
    }

    @Override
    public ResponseEntity<UserResponse> register(@Valid RegisterRequest registrationRequest) {
        log.debug("register invoked username={}", registrationRequest.getUsername());

        UserResponse createdUserProfile = authService.register(registrationRequest);
